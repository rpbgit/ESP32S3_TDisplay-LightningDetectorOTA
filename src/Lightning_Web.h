...
// zv declare below        var xmldoc;
        ...
        try {
            if (xmldoc.length > 0 ){ //&& xmldoc[0].firstChild) {
                ...
                // we dont always have the INFO tag in the XML message, so this is ok
                // so comment out warning console.warn("INFO tag is missing or empty");
            }
        } catch (e) {
...
//            playStrikeAlarm();  // finally, play the strike alarm tone
        ...
        xmldoc = xmlResponse.getElementsByTagName("VER");
        if (xmldoc.length > 0) { // Check if there are elements in the array
            message = xmldoc[0].firstChild.nodeValue;
            document.getElementById("headerTitle").textContent = "ESP32 Storm detector " + message;
        }
