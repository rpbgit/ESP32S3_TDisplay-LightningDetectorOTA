...
<head>
  ...
  <style>
    ...
    /* Pastel green for toggleLog button when logging is enabled */
    .logging-enabled {
    ...
    /* Modal dialog for simplified log actions */
    .modal {
        display: none;
        position: fixed;
        top: 0;
...
<body style="background-color: #7a8285" onload="OnMyFormLoad()">
  ...
  <main class="main-content">
    ...
    <section class="volume-control-container">
      <input type="button" class="button" id="PWR_Select" name="PWR_Select" value="POWER" onclick="PowerButtonHandler(this.id)">
      <input type="button" class="button" id="toggleLog" value="Enable Logging">
      <input type="button" class="button" id="R2Select" name="Radio2" value="Test" onclick="TestButtonHandler(this.id)">
  ...
  <!-- Modal dialog for simplified log actions -->
  <div id="logActionModal" class="modal">
    <div class="modal-content">
      <h2>Logging Options</h2>
      <p>Logging is enabled. Choose an action to disable logging:</p>
      <form id="logActionForm">
        <label>
          <input type="radio" name="logAction" value="downloadAndClear" checked>
          Download and clear logs
        </label>
        <label>
          <input type="radio" name="logAction" value="disable">
          Disable logging
        </label>
        <label>
          <input type="radio" name="logAction" value="cancel">
          Cancel
        </label>
        <div class="button-container">
          <button type="submit" class="submit">OK</button>
          <button type="button" class="cancel" onclick="closeLogActionModal()">Cancel</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <script type="text/javascript">
    ...
    const STORAGE_KEY = 'stormDetectorLogs'; // Key for localStorage
    const MAX_STORAGE_SIZE = 256 * 1024; // 256 KB limit
    let logEntries = []; // Array of log messages
    let isLoggingEnabled = false; // Logging state
    let avgEntrySize = 50; // Estimated average log entry size (bytes, UTF-16)

    // Load existing logs from localStorage
    try {
      const stored = localStorage.getItem(STORAGE_KEY);
      if (stored) {
        logEntries = stored.split('\n').filter(entry => entry.trim() !== '');
        if (logEntries.length > 0) {
          const totalSize = stored.length * 2 + STORAGE_KEY.length * 2;
          avgEntrySize = Math.max(20, Math.min(200, totalSize / logEntries.length));
        }
      }
    } catch (e) {
    ...
    // Calculate total localStorage size (bytes, UTF-16)
    ...
    // Save log entries to localStorage with FIFO pruning
    function saveToStorage(entries, newEntrySize) {
      try {
        let data = entries.join('\n'); // Join entries with newlines
        let newSize = getStorageSize() + (data.length * 2) - (localStorage.getItem(STORAGE_KEY)?.length * 2 || 0);
        
        if (newSize > MAX_STORAGE_SIZE && entries.length > 0) {
          // Batch prune based on estimated overflow
          const overflow = newSize - MAX_STORAGE_SIZE;
          const entriesToRemove = Math.max(1, Math.ceil(overflow / avgEntrySize));
          const removed = entries.splice(0, entriesToRemove);
          
          data = entries.join('\n');
          newSize = getStorageSize() + (data.length * 2) - (localStorage.getItem(STORAGE_KEY)?.length * 2 || 0);
          
          if (newSize > MAX_STORAGE_SIZE) {
            console.error('Storage limit (256 KB) exceeded after pruning.');
            entries.unshift(...removed); // Revert if pruning fails
            return false;
          }
          
          // Update average entry size
          avgEntrySize = (avgEntrySize * (entries.length + removed.length) + newEntrySize - removed.reduce((sum, entry) => sum + entry.length * 2, 0)) / (entries.length + 1);
          avgEntrySize = Math.max(20, Math.min(200, avgEntrySize));
          
          localStorage.setItem(STORAGE_KEY, data);
          return true;
        }
        
        // Update average entry size for normal case
        if (entries.length > 1) {
          avgEntrySize = (avgEntrySize * (entries.length - 1) + newEntrySize) / entries.length;
          avgEntrySize = Math.max(20, Math.min(200, avgEntrySize));
        }
    ...
    // Add a message to logs and storage
    function AddToLog(message) {
      ...
      if (!saveToStorage(logEntries, message.length * 2 + 2)) { // +2 for \n
        logEntries.pop();
        return false;
      }
      return true;
    ...
    function clearLogs() {
      logEntries = [];
      avgEntrySize = 50; // Reset estimate
      try {
    ...
    // Show modal dialog for log actions
    ...
    // Handle toggle button for enabling/disabling logging
    const toggleButton = document.getElementById('toggleLog');
    toggleButton.addEventListener('click', () => {
      // Toggle pastel green background when enabling/disabling logging
      if (!isLoggingEnabled) {
        isLoggingEnabled = true;
        toggleButton.value = 'Disable Logging';
        toggleButton.classList.add('logging-enabled');
    ...
    // Handle simplified modal form submission (download and clear, disable, cancel)
    document.getElementById('logActionForm').addEventListener('submit', (event) => {
      event.preventDefault();
      const action = document.querySelector('input[name="logAction"]:checked').value;
      closeLogActionModal();
      
      if (action === 'downloadAndClear') {
        if (downloadLog()) {
          clearLogs();
          alert('Logs downloaded and cleared.');
        } else {
          alert('No logs to download, logs not cleared.');
        }
        isLoggingEnabled = false;
        toggleButton.value = 'Enable Logging';
        toggleButton.classList.remove('logging-enabled');
        ...
        toggleButton.value = 'Enable Logging';
        toggleButton.classList.remove('logging-enabled');
        alert('Logging disabled and logs cleared.');
        console.log('Logging disabled.');
      }
      // 'cancel' does nothing, modal closes, logging remains enabled
    });
    // --------------------- Web Client Logging END ------------------------------

    // try to emulate an autoscrolling log using a text area
    function TextLog(value) {
      var dt = new Date();
      var ta = document.getElementById("Terminal");
      var message = dt.toLocaleTimeString() + "> " + value;
      ta.value += "\n" + message;
    ...
    function response() {
      ...
      if (jsonResponse.STRIKE_ACC !== undefined) {
        ...
        // the following code is used to determine when to play the strike alarm tone
        ...
        if (Strike_this_time > Strike_last_strike_accumulator_value) {
          if (!USE_SIMULATED_DATA) {
            ...
            Strike_last_strike_accumulator_value = Strike_this_time; // save the last strike accumulator value
          } else {
            TextLog(">SIMBEEP<"); // so I don't have to listen to the alarm tone when testing
            Strike_last_strike_accumulator_value = 0; // reset the strike accumulator
          }
        }
      }
      ...
      if (!USE_SIMULATED_DATA && Array.isArray(jsonResponse.RATES)) {
        ...
        const now = Date.now();
        if (now - lastChartUpdateTime < chartUpdateRateInMs) return; // if it's not time to update, return do nothing
    ...
    // Function to play strike alarm tone
    ...
    // Function to play generic alarm sound
          ...
          plugins: {
            legend: {
              labels: {
                ...
                boxWidth: 16,                     // Reduce for smaller boxes
                boxHeight: 8,                     // Reduce for smaller boxes
            ...
            title: {
              ...
              color: '#d4d4d4',
              font: {
                size: 14,      // Reduce from 20 to 14
                weight: 'bold'
              },
 takdir: { top: 4, bottom: 6 } // Reduce padding above/below title
            }
          },
          scales: {
            x: {
              ...
              grid: {
                color: 'rgba(136,136,136,0.49)',      // Dimmed grid lines
                lineWidth: 1,
                drawBorder: true,
      ...
      // Optionally, start simulated data updates here after the chart is created
      if (USE_SIMULATED_DATA) {
    ...
    // Update chart with new rate values
