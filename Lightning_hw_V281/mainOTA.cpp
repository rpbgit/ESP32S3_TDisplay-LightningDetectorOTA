...
// TODO: flashify all these string const's
    ...
    // copy the version string into the xml message to send back to the web page
   
    // Assuming XML_buffer is large enough to hold the resulting string.
    snprintf(XML_buffer + strlen(XML_buffer), sizeof(XML_buffer) - strlen(XML_buffer),
         "<VER>%s</VER>", RAS_Status.pSoftwareVersion);

    //strcat(XML_buffer, "<VER>" + RAS_Status.pSoftwareVersion + "</VER>\n");
    ...
    if( RingBuffer.isEmpty() == false ) {
        strcat(XML_buffer+strlen(XML_buffer), "<INFO>" );   // append open tag
        RingBuffer.concat_all(XML_buffer+strlen(XML_buffer));  // append the contents of the RingBuffer into the XML response buffer.
        strcat(XML_buffer+strlen(XML_buffer), "</INFO>\n");  // append close the INFO tag
    }
    RingBuffer.delete_all();  // im done with it
...
//Serial.printf("\n\nYo XML >>>>:%s<<<<\n\n",XML_buffer);
    ...
    // static int x = 0;
    // if(x < 4 ){
    //     Serial.println(XML_buffer);
    //     x++;
    // }

    request->send(200, "text/xml", XML_buffer);

    XML_buffer[0] = 0; // make sure for the next time.
}

void buildXmlTag(char *buffer, size_t bufferSize, const char *tagName, unsigned long value) {
    // Safely format the XML tag with the integer value inside
