...
constexpr const char* CODE_VERSION_STR = "v2.81";  // a string for the application version number
...
{
    int faked_event = 0; // zero this out every time around the patch
    int fake_distance = 0;
    long fake_energy = 0;
    ...
    if(gSimulated_Events) {
        if( now - last_event >= gSimulated_Events ) {  // every FAKED_DATA_INTERVAL seconds we "might" generate an event
            ...
            // if(faked_event == 1 || faked_event == 4 || faked_event == 8)
            //   Serial.printf(">>>> fake event generated %0x\n", faked_event);
            Serial.print(".");
            last_event = now;
        }
    }

    if ( gEvents_Active_Flag == true && (digitalRead(DetectorIntrReqPin) == HIGH || faked_event) ) {
...
//Serial.printf("INTERRUPT SOURCE REG VALUE: %#04x\n", interrupt_reg_value);
        if (interrupt_reg_value == NOISE_INT ) {
            static unsigned int noise_accumulator = 0;
            static unsigned long noise_last = 0;  // its a static, initialize it...
            if(noise_accumulator == 0) noise_last = now;  // init noise_last if first event
            noise_accumulator++; // we got one a fish on the line, increment the count.
            unsigned long elapsed = ((now - noise_last)/1000 ) % 10000; // only have screen space for 4 digits
            //Serial.printf("Noise - ET since last %d, accumulated events since reset %d\n", elapsed, noise_accumulator );
            WebText("Noise - accumulated events since reset %d, ET since last %d\n", noise_accumulator, elapsed  );
            
            // update the structure that is used to build XML response message
            ...
            noise_last = now;// save it for next time...
        } else if (interrupt_reg_value == DISTURBER_INT) {
            static unsigned long disturber_accumulator = 0;
            static unsigned long disturber_last = 0;  // its a static, initialize it...
            if(disturber_accumulator == 0) disturber_last = now; // first time, init it
            disturber_accumulator++; // we got one, increment the count.
            unsigned long elapsed = ((now - disturber_last)/1000) % 10000; // only have screen space for 4 digits
            ...
            tft.setTextPadding(tft.width());  // pad it the width of the screen.
            // print it out on the tft, and calculate the ET in seconds since last one
            tft.printf("Disturber: %4d Last  %4d", disturber_accumulator, elapsed ); 
            disturber_last = now;// save it for next time...

        } else if (interrupt_reg_value == LIGHTNING_INT) {
            static unsigned long stroke_accumulator = 0;
            static unsigned long stroke_last = 0;
            if(stroke_accumulator == 0) stroke_last = now; // first time, init it
            stroke_accumulator++;  //bump the stroke counter
            unsigned long elapsed = ((now - stroke_last)/1000) % 10000;  // only have screen space for 4 digits
            ...
            // conv to miles, device register output is in km, avoid floating point math
            // mult by conv factor, and scale down by 1000000 to get the miles value,
            distance = ((long)distance * 621371L) / 1000000L;
            // handle the case where device sez 1 km so we dont display 0
            if(distance == 1  || distance == 0) {  // should never be zero unless autogen data
                tft.printf(" Approx <= 1 mile distant"); 
                WebText("\tApprox <= 1 mile distant ", distance);
            } else {
                tft.printf(" Approx  %3d miles distant", distance); 
                WebText("\tApproximately: %d mi away!\n", distance);
            }
            ...
            lightEnergy = faked_event ? fake_energy : lightEnergy;
            ...
            stroke_last = now; // save it for next time...
           Sensor.clearStatistics(); // clear the statistics and strike reg accumulation, so we can get a new integ count for the next event.
...
void get_hardware_status(RAS_HW_STATUS_STRUCT& myhw ) 
{
    myhw.pSoftwareVersion = (char*) CODE_VERSION_STR;
...
void handle_AFE_Command(char* param)
{
    ...
    WebText("\t- AFE indoor/outdoor is %s\n", Sensor.readAFE() == 0x0E ? "OUTDOOR" : "INDOOR");
}
...
// void power_relay_set_reset(HostCmdEnum& host_command) {
//     static bool bArmed = true;              // Indicates if the relay action is allowed
//     static long lastEventTime = 0;         // Tracks the last event time for debounce

//     const long debounceDelay = 200;        // Minimum delay (200ms) between actions

//     bool buttonPressed = (digitalRead(PIN_BUTTON_2) == LOW); // Button active state
//     bool relayState = (digitalRead(RELAY_SENSE) == HIGH);    // Current relay state

//     // Check debounce timer
//     if ((millis() - lastEventTime) < debounceDelay) {
//         return;  // Exit if debounce period has not elapsed
//     }

//     if (bArmed && (host_command == HostCmdEnum::PWR_SELECT || buttonPressed)) {
//         // Toggle relay state if armed
//         if (relayState) {
//             Serial.println("Relay turned OFF");
//             digitalWrite(RELAY_SENSE, LOW);  // Turn relay OFF, replade with your relay control function
//         } else {
//             Serial.println("Relay turned ON");
//             digitalWrite(RELAY_SENSE, HIGH); // Turn relay ON, replade with your relay control function
//         }

//         bArmed = false;  // Disarm until the button/host command is released
//         lastEventTime = millis();  // Update the debounce timer
//     }

//     // Re-arm logic: Re-arm only when button is released or host command cleared
//     if (!buttonPressed && host_command == HostCmdEnum::NO_OP) {
//         bArmed = true; // Re-enable relay actions
//     }
// }

/* void power_relay_set_reset(HostCmdEnum& host_command) // Power On/Off Hardware function.
{
    static bool relay_flag = false; // Power Relay On/Off flag, static allows it to retain its value between function invocations.
    static bool host_flag = false; // Host On/Off flag, static allows it to retain its value between function invocations.

    static bool bArmed; // holds off listening for commands until we get a transition
    if (host_command == HostCmdEnum::PWR_SELECT && bArmed == true) {
        if (digitalRead(RELAY_SENSE_STATE) == HIGH) { // if the power is on
            // code to turn it OFF if its on - i dont know if high/low is ON, whatever it needs to be
            power_relay_control(PWR_OFF);
            Serial.println("Relay turned OFF");
        } else {
            // code to turn it
            power_relay_control(PWR_ON);
            Serial.println("Relay turned ON");
        }
        // set this armed state to false such that we ignore further PWR_Selects UNTIL it the next one
        bArmed = false;
    } else if (host_command == HostCmdEnum::NO_OP) {
        // re-arm the trigger after we see the host command clear, wait for the next one
        bArmed = true; // re-enable listening for a PWR_Select command
    }

    if (relay_flag == true) { // && digitalRead(RELAY_SENSE_STATE) == LOW) {
        if (digitalRead(PIN_BUTTON_2) == LOW) { // Turn on the power
            power_relay_control(PWR_ON);
            if (digitalRead(PIN_BUTTON_2) == HIGH) {
                relay_flag = false; // the relay flag is set to true or armed when the button is released.
                //   pRas->PWR_Button = 1;
                WebText("\tHW_Power is ON\n");
            }
        }
    }
    if (relay_flag == false) { // && digitalRead(RELAY_SENSE_STATE) == HIGH) {
        if (digitalRead(PIN_BUTTON_2) == LOW) { // Turn  Off the power
            power_relay_control(PWR_OFF);
            if (digitalRead(PIN_BUTTON_2) == HIGH) {
                relay_flag = true; // the relay flag is set to false or unarmed when the button is released.
                //  pRas->PWR_Button = 0;
                WebText("\tHW_Power is OFF\n");
            }
        }
    }
} */